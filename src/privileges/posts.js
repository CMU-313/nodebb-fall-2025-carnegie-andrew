...
privsPosts.get = async function (pids, uid) {
	...
	const privileges = cids.map((cid, i) => {
		...
		const editable =
			(privData['posts:edit'][cid] && (results.isOwner[i] || results.isModerator[i])) ||
			results.isAdmin;
		const viewDeletedPosts =
			results.isOwner[i] || privData['posts:view_deleted'][cid] || results.isAdmin;
...
privsPosts.canFlag = async function (pid, uid) {
	...
	const [userReputation, isAdminOrModerator, targetPrivileged, reporterPrivileged] =
		await Promise.all([
