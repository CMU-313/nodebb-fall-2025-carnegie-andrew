				...
				app.parseAndTranslate(
					...
					function (html) {
						...
						function saveSelection() {
							...
							const iconBgColor =
								modal.find('[data-bg-color].selected').attr('data-bg-color') || 'transparent';
	...
	Picture.updateHeader = (picture, iconBgColor) => {
		...
		if (picture) {
			if (!headerPictureEl.length && headerIconEl.length) {
				...
				img.attr('component', 'avatar/picture').attr('src', picture).insertBefore(headerIconEl);
			}
		} else {
	...
	function handleImageUpload(modal) {
		function onUploadComplete(urlOnServer) {
			...
			if (ajaxify.data.picture && ajaxify.data.picture.length) {
				$(`#user-current-picture, img[data-uid="${ajaxify.data.theirid}"].avatar`).attr(
					'src',
					cacheBustedUrl,
				);
