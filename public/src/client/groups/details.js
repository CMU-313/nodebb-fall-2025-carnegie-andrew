...
define('forum/groups/details', [
			...
			switch (action) {
				...
				case 'kick':
					bootbox.confirm('[[groups:details.kick-confirm]]', function (confirm) {
						...
						api
							.del(`/groups/${ajaxify.data.group.slug}/membership/${uid}`, undefined)
							.then(() => {
								userRow.remove();
				...
				case 'join':
					api
						.put(
							'/groups/' + ajaxify.data.group.slug + '/membership/' + (uid || app.user.uid),
							undefined,
						)
				...
				case 'rejectInvite':
					api
						.del(`/groups/${ajaxify.data.group.slug}/invites/${uid || app.user.uid}`)
						.then(() => {
							if (uid) {
								userRow.remove();
				...
				case 'rejectAll': {
					...
					let uids = Array.prototype.map.call(listEl.querySelectorAll('[data-uid]'), el =>
						parseInt(el.getAttribute('data-uid'), 10),
					);
					uids = uids.filter((uid, idx) => uids.indexOf(uid) === idx);

					Promise.all(
						uids.map(async uid => api[method](`/groups/${ajaxify.data.group.slug}/pending/${uid}`)),
					)
						.then(() => {
							ajaxify.refresh();
						})
	...
	function handleMemberInvitations() {
		...
		async function updateList() {
			...
			const html = await app.parseAndTranslate('groups/details', 'group.invited', {
				group: data.group,
			});
		...
		require(['autocomplete'], function (autocomplete) {
			autocomplete.user(searchInput, function (event, selected) {
				api
					.post(`/groups/${ajaxify.data.group.slug}/invites/${selected.item.user.uid}`)
					.then(() => updateList())
					.catch(alerts.error);
			});
		});
		...
		$('[component="groups/members/bulk-invite-button"]').on('click', async () => {
			...
			const uids = await Promise.all(
				usernames.map(slug => api.get(`/users/bySlug/${slug}`).then(({ uid }) => uid)),
			);
